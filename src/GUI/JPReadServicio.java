...
public class JPReadServicio extends javax.swing.JPanel {
    ControladoraLogica controladoraL = new ControladoraLogica();
    JPAddServicio addServicio = null;
    JPUpdateServicio updateServicio = null;
    public JPReadServicio() {
        initComponents();
        cargarTabla();
    }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("REVISAR SERVICIOS");
    ...
    private void jBAgregarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBAgregarActionPerformed
        addServicio = new JPAddServicio();
        addServicio.setSize(824, 416);
        addServicio.setLocation(0,0);
        
        this.removeAll();
        this.add(addServicio,BorderLayout.CENTER);
        this.revalidate();
        this.repaint();
    }//GEN-LAST:event_jBAgregarActionPerformed

    private void jBModificarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBModificarActionPerformed
        if(jTServicio.getRowCount() > 0){
            if (jTServicio.getSelectedRow() != -1) {
                int idServicio = Integer.parseInt(String.valueOf(jTServicio.getValueAt(jTServicio.getSelectedRow(), 0)));

                updateServicio = new JPUpdateServicio(idServicio);

                updateServicio.setSize(824, 416);
                updateServicio.setLocation(0, 0);

                this.removeAll();
                this.add(updateServicio, BorderLayout.CENTER);
                this.revalidate();
                this.repaint();
            } else {
                JOptionPane.showMessageDialog(null, "No selecciono ningun servicio", "Error al modificar",JOptionPane.ERROR_MESSAGE);
            }
        } else{
    ...
    private void jBeliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBeliminarActionPerformed
        if(jTServicio.getRowCount() > 0){
            if (jTServicio.getSelectedRow() != -1) {
                ...
                controladoraL.eliminarServicio(idServicio);
                
                JOptionPane.showMessageDialog(null, "Servicio eliminado exitosamente", "Informacion",JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(null, "No selecciono ningun servicio", "Error al modificar",JOptionPane.ERROR_MESSAGE);
            }
        } else{
    ...
    private void cargarTabla() {
        ...
        String titulos[] = {"ID", "Tipo", "Descripcion", "Costo", "Disponibilidad"};
        modeloTabla.setColumnIdentifiers(titulos);
        
        
        //Carga de datos desde la BD
        List <Servicio> listaServicios = controladoraL.traerServicios();
        
        //Recorrer la lista y mostrar las tuplas en la tabla
        if (listaServicios != null) {
            for (Servicio servi : listaServicios) {
                Object[] atributos = {servi.getIdServicio(), servi.getTipoServicio(), servi.getDescripcion(), formatoMoneda.format(servi.getCosto()), servi.getDisponibilidad()};
