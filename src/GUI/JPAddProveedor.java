        ...
        if (!nombre.isEmpty() && !ubicacion.isEmpty() && !telefono.isEmpty() && !email.isEmpty()) {
            ...
            if (telefono.matches("\\d{10}")) {
                ...
                if (isValidEmail(email)) {
                    controladoraL.guardarProveedor(nombre, ubicacion, telefono, email);

                    JOptionPane.showMessageDialog(null, "Proveedor agregado exitosamente", "Informacion", JOptionPane.INFORMATION_MESSAGE);
                    regresarTabla();
                } else {
